...
class ThreadedStreamHandler extends Thread {
	 ...
	 * @param streamType
	 */
	ThreadedStreamHandler(InputStream inputStream, boolean dumpToConsole, String threadName) {
	...
	ThreadedStreamHandler(InputStream inputStream, OutputStream outputStream,
			String adminPassword, boolean dumpToConsole, String threadName) {
		this.inputStream = inputStream;
	...
	public void run() {
		...
		try {
			...
			while ((line = bufferedReader.readLine()) != null) {
				...
				if (dumpToConsole) {
					System.out.println(Formatter.format(line,threadName));
				}
			}
